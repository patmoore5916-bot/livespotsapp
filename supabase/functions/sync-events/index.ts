...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (scraperApiKey) fetchHeaders["Authorization"] = `Bearer ${scraperApiKey}`;
    ...
    for (const v of scraperVenues) {
      ...
      if (existing) {
        await adminClient.from("venues").update({
          type: v.type ?? "venue",
        ...
        const { data: inserted } = await adminClient.from("venues").insert({
          name: v.name,
          city: v.city,
